...
def render_graph(G, selected_node=None, deviation_only=False, trace_mode="None"):
    ...
    # --- Add nodes with enhanced hover tooltip ---
    for node_id, data in G.nodes(data=True):
        ...
        if data["type"] == "Batch":
            shape, size, color = "star", 50, "#f39c12"
        elif data["type"] == "Phase":
            shape, size, color = "triangle", 35, "#2ecc71"
        elif data["type"] == "PI":
            shape, size, color = "box", 30, "#e74c3c" if data.get("result")=="FAIL" else "#9b59b6"
        elif data["type"] == "Material":
            shape, size, color = "ellipse", 25, "#34495e"
        if node_id == selected_node:
            shape, size, color = "diamond", 45, "#f1c40f"

        # Enhanced tooltip with parameters, limits, timestamps, deviations
        tooltip_lines = [f"<b>{data['label']}</b>", f"Type: {data['type']}"]
        for k in ["parameters","limits","timestamp","deviation"]:
            if k in data and data[k]:
                tooltip_lines.append(f"{k}: {json.dumps(data[k])}")
    ...
    for u, v, data_edge in G.edges(data=True):
        ...
        if trace_mode != "None" and u in highlight_set and v in highlight_set:
            edge_color, width = "#2c3e50", 4
        if data_edge["relationship"] in ["consumed_by","produced"]:
            arrows = "to"
...
#         net.add_node(node_id, label=data["label"], shape=shape, size=size, color=color, opacity=opacity, title=tooltip)
